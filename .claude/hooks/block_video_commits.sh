...
# outside the allowed sample. Deterministic guard for the sideOut rule:
# raw video is NEVER committed. Exit 2 blocks the tool call; exit 0 allows.
...
BAD=$(git diff --cached --name-only 2>/dev/null | grep -iE '\.(mp4|mov|avi|mkv|m4v|webm)$' | grep -vE '^samples/demo\.(mp4|mov)$')
if [ -n "$BAD" ]; then
